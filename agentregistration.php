...
<?php
    ...
    if (isset($_REQUEST['LoginId'])) {
        ...
        $query    = "INSERT into `agentinfo` (LoginId, UserPassword, FName, LName, CellNumber,  EmailAddress, rollId, city, pincode)
                     VALUES ('$LoginId', '" . md5($UserPassword) . "', '$FName', '$LName', '$CellNumber',  '$EmailAddress', '$rollId', '$city', '$pincode')";
        $result   = mysqli_query($con, $query);
        if ($result) {
            echo "<div class='form'>
                  <h3>You are registered successfully.</h3><br/>
                  <p class='link'>Click here to <a href='agentlogin.php'>Login</a></p>
                  </div>";
        } else {
            echo "<div class='form'>
                  <h3>Required fields are missing.</h3><br/>
                  <p class='link'>Click here to <a href='agentregistration.php'>registration</a> again.</p>
                  </div>";
        }
    } else {
?>
